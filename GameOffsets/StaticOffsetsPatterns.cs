    ...
    public struct StaticOffsetsPatterns
    {
        public static readonly Pattern[] Patterns =
        {
            ...
            new(
                ...
                //OLD "48 83 EC ?? 48 8B F1 33 ED 48 39 2D ^ ?? ?? ?? ??"
            ),
            ...
            new(
                ...
                //OLD "C7 ?? 00 00 80 3F C7 ?? 04 00 00 80 3F C3 ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? C3 ?? ?? ?? ?? ^ ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? C3 ?? ?? ?? ??"
            ),
            ...
            new(
                ...
                //OLD "48 8d ?? ^ ?? ?? ?? ?? 41 ?? ?? ?? ?? ?? 48 8d ?? ?? ?? 45 ?? ?? 0f ?? ?? ?? 48 ?? ?? ?? ?? 88 ?? ?? ??"
            ),
