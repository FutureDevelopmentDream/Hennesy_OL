...
namespace Vision
{
    ...
    /// <summary>
    /// <see cref="Vision"/> plugin.
    /// </summary>
    public sealed class VisionCore : PCore<VisionSettings>
    {
        ...
        //private readonly string hennesyMonsterStarting = "Metadata/Monsters/Daemon/";
        //private readonly Dictionary<uint, string> hennesyMonsterCache = new();
        /// <summary>
        ...
        public override void DrawSettings()
        {
            ...
            ImGui.Separator();
            ImGui.NewLine();
            ImGui.Checkbox("Show NPCs name + Buffs", ref this.Settings.ShowNPCNames);
            if(ImGui.ColorEdit4("NPCs Name Color", ref this.Settings.NPCColor))
            {
                if (this.walkableMapTexture != IntPtr.Zero)
                {
                    this.ReloadMapTexture();
        ...
        public override void OnEnable(bool isGameOpened)
        {
            ...
            if (File.Exists(this.SettingPathname))
            {
                ...
                this.Settings = JsonConvert.DeserializeObject<VisionSettings>(content);
            }
        ...
        private void DrawMapIcons(Vector2 mapCenter, float iconSizeMultiplier)
        {
            ...
            foreach (var entity in currentAreaInstance.AwakeEntities)
            {
                ...
                var curPos = entityRender.WorldPosition;
                curPos.Z -= entityRender.ModelBounds.Z;
                ...
                switch (entity.Value.EntityType)
                {
                    ...
                    case EntityTypes.Monster:
                        ...
                        /*if (omp.Rarity == Rarity.Rare)
                        {
                            var entname = entity.Value.Path.Split('/').LastOrDefault();

                            fgDraw.AddText(mapCenter + fpos, ImGuiHelper.Color(255, 255, 255, 255), entname);

                            if (entname == "FireProjectileRetaliationDaemon")
                            {
                                var customIcon = SelectCustomIcon();
                                iconSizeMultiplierVector *= customIcon.IconScale;

                                fgDraw.AddImage(
                                    customIcon.TexturePtr,
                                    mapCenter + fpos - iconSizeMultiplierVector,
                                    mapCenter + fpos + iconSizeMultiplierVector,
                                    customIcon.UV0,
                                    customIcon.UV1);
                        ...
                        /** if (this.hennesyMonsterCache.TryGetValue(entity.Key.id, out var iconFinder2))
                        {
                            if (this.Settings.HennesyIcons.TryGetValue(iconFinder2, out var hennesyMonsterIcon))
                            {
                                if (hennesyMonsterIcon.UV0 == Vector2.Zero)
                                {
                                    var s = ImGui.CalcTextSize(iconFinder2) / 2;
                                    fgDraw.AddRectFilled(mapCenter + fpos - s, mapCenter + fpos + s,
                                        ImGuiHelper.Color(0, 0, 0, 255));
                                    fgDraw.AddText(mapCenter + fpos - s, ImGuiHelper.Color(255, 128, 128, 255),
                                        iconFinder2);
                                ...
                                {
                                    iconSizeMultiplierVector *= hennesyMonsterIcon.IconScale;
                                    fgDraw.AddImage(
                                        hennesyMonsterIcon.TexturePtr,
                                        mapCenter + fpos - iconSizeMultiplierVector,
                                        mapCenter + fpos + iconSizeMultiplierVector,
                                        hennesyMonsterIcon.UV0,
                                        hennesyMonsterIcon.UV1);
                                }
                            }
                        }
                        else
                        {
                            ...
                            this.hennesyMonsterCache[entity.Key.id] =
                                this.HennesyMonsterPathToIcon(entity.Value.Path);
                        } **/
                        ...
                        //Customs
                        if (this.Settings.ShowNPCNames)
                        {
                            
                            entity.Value.TryGetComponent<Buffs>(out var buffs__);
                            //entity.Value.TryGetComponent<ObjectMagicProperties>(out var objm);

                            //var qcqc = entity.Value.TryGetComponent
                            //buffs__.TryGetNameFromBuffDefination2(buffs__.Address,out var sd);
                            var name__ = (entity.Value.Path.Split('/').LastOrDefault().Contains("@") ? entity.Value.Path.Split('/').LastOrDefault().Split("@")[0] : entity.Value.Path.Split('/').LastOrDefault());
                            //name__ += ($"{objm.Mods.First().Value.Name}\n{ name__}");
                            if (buffs__.StatusEffects.Count > 0)
                            {
                                name__ += $"[Buffs:{ buffs__.StatusEffects.Count }]";

                                foreach (var d in buffs__.StatusEffects)
                                {
                                    name__ += $"\n[{d.Key}]";
                                }
                                    
                            }
                            var st = ImGui.CalcTextSize(name__) / 2;
                            st.Y -= 5f;
                            fgDraw.AddText(location - st, ImGuiHelper.Color(255, 128, 128, 255), name__);
                        }
        ...
        /*private IconPicker SelectCustomIcon()
        {
            return this.Settings.HennesyIcons[$"FireProjectileRetaliationDaemon"];
        }*/
        ...
        /*private string HennesyMonsterPathToIcon(string path)
        {
            return path.Replace(this.hennesyMonsterStarting, null, StringComparison.Ordinal);
        }*/
        ...
        private void CleanUpRadarPluginCaches()
        {
            ...
            //this.hennesyMonsterCache.Clear();
            this.RemoveMapTexture();
